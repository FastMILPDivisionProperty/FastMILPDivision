...
public:
    ...
    friend void removeUnusedTrails(uint64_t in64, uint64_t out64, std::vector<std::vector<DivTable>> & vdiv, std::vector<uint8_t> const & perm);
    friend void removeUnnecessaryTrails(std::vector<std::vector<DivTable>> & vdiv, std::vector<uint8_t> const & perm);
    friend void removeUnusedTrailsIN(uint64_t in64, std::vector<std::vector<DivTable>> & vdiv, std::vector<uint8_t> const & perm);
    friend void removeUnusedTrailsOUT(uint64_t out64, std::vector<std::vector<DivTable>> & vdiv, std::vector<uint8_t> const & perm);
private:
