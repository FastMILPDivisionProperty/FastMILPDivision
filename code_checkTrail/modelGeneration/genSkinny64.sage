...
def genSkinny64Model(rMax, sboxModel="Hull", linModel="Lbox", lastLin=True, smartLin=True, firstSbox=True):
	"""
	...
	Pbit = [0,1,2,3,20,21,22,23,40,41,42,43,60,61,62,63,16,17,18,19,36,37,38,39,56,57,58,59,12,13,14,15,32,33,34,35,52,53,54,55,8,9,10,11,28,29,30,31,48,49,50,51,4,5,6,7,24,25,26,27,44,45,46,47]
	...
	anfS = [x0*x1*x2 + x0*x1 + x0*x2 + x0*x3 + x1*x2*x3 + x1*x3 + x1 + x2 + x3,
	 		x0*x1 + x0 + x1*x2*x3 + x1*x2 + x1*x3 + x2*x3 + x3,
	  		x1*x2 + x1 + x2 + x3 + 1,
	  		x0 + x2*x3 + x2 + x3 + 1]
	...
	#Create the model
	modelName = "Skinny64_"+str(rMax)+"r_"+sboxModel+"_"+linModel
	if lastLin:
		modelName += "_lastLin"
...
if __name__ == "__main__":
	...
	genSkinny64Model(rMax,sboxModel,linModel,lastLin,smartLin,firstSbox)